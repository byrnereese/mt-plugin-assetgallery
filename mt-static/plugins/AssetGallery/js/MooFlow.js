...
var MooFlow = new Class({
	...
	addLoader: function(){
	    this.MooFlow.setStyle('display', 'block');
		this.MooFlow.store('height', (this.MooFlow.getSize().x*this.options.heightRatio) + 50);
	...
	update: function(e){
		...
		this.oH = this.MooFlow.getSize().x*this.options.heightRatio;
		this.sz = this.oW * 0.5;
		if(this.options.useSlider){	
	...
	process: function(x){
		var zI=this.iL,z,W,H,foc=this.foc,f=this.factor,sz=this.sz,oW=this.oW,oH=this.oH,div;
		with (Math) {
			this.master.images.each(function(el){
				div = el.div;
				if(x>-foc*6 && x<foc*6){
					...
					div.setStyle('top', round(oH - (H*1.2))  + 'px');							
